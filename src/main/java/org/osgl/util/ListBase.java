...
public abstract class ListBase<T> extends AbstractList<T> implements C.List<T> {
    ...
    public C.List<T> copy() {
        return C.Mutable.List(this);
    }

    @Override
    public C.List<T> sorted() {
        if (size() == 0) return C.Mutable.List();
        T t = get(0);
        C.List<T> l = copy();
    ...
    public C.List<T> unique(Comparator<T> comp) {
        Set<T> set = null == comp ? new HashSet<T>() : new TreeSet<>(comp);
        ...
        for (T t: this) {
            ...
            if (set.contains(t)) {
                if (null == retList) {
                    retList = C.Mutable.sizedList(size());
    ...
    public C.List<T> take(int n) {
        ...
        C.List<T> l = C.Mutable.sizedList(n);
    ...
    public C.List<T> takeWhile($.Function<? super T, Boolean> predicate) {
        ...
        if (immutable) {
            ...
            C.List<T> l = C.Mutable.sizedList(sz);
            for (T t : this) {
                if (predicate.apply(t)) {
                    l.add(t);
                } else {
    ...
    public C.List<T> drop(int n) throws IndexOutOfBoundsException {
        ...
        if (n < 0) {
            n = -n;
            if (n >= sz) {
                return immutable ? C.<T>List() : C.Mutable.<T>List();
            } else {
                return take(sz - n);
            }
        }
        ...
        C.List<T> l = C.Mutable.sizedList(sz - n);
    ...
    public <R> C.List<R> flatMap($.Function<? super T, ? extends Iterable<? extends R>> mapper
        ...
        if (immutable) {
            ...
            C.List<R> l = C.Mutable.sizedList(sz * 3);
    ...
    public C.List<T> insert(int index, T t) throws IndexOutOfBoundsException {
        ...
        if (isImmutable()) {
            ...
            C.List<T> l = C.Mutable.sizedList(sz + 1);
            if (index > 0) {
    ...
    public C.List<T> without(T element, T... elements) {
        ...
        C.List<T> lr = C.Mutable.sizedList(l.size());
        if (c) {
            for (T t : l) {
    ...
    private C.List<T> unLazyAppend(Iterable<? extends T> iterable) {
        ...
        C.List<T> l = C.Mutable.sizedList(size() * 2);
        l.addAll(this);
        l.addAll(iterable);
        return l;
    }
    ...
    public C.List<T> append(T t) {
        ...
        C.List<T> l = C.Mutable.sizedList(size() + 1);
        l.addAll(this);
        l.add(t);
        return l;
    }
